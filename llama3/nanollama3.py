...
class Transformer(nn.Module):
    ...
    def forward_train(self, inputs: torch.Tensor, targets: torch.Tensor, ignore_index=-100):
...
def read_dataset(path: str, tokenizer: Tokenizer, max_seq_len: int):
    ...
    for i in range(0, len(raw_text_enc), max_seq_len):
        ...
        if len(enc_item) < max_seq_len + 1:
            enc_item = enc_item + [tokenizer.pad_id] * (max_seq_len + 1 - len(enc_item))
        enc_data.append(enc_item)
    return enc_data

def main(
    ckpt_dir: str = "/home/zzz/model/Meta-Llama-3-8B-Instruct/original",
    tokenizer_path: str = "/home/zzz/model/Meta-Llama-3-8B-Instruct/original/tokenizer.model",
    ...
    dataset = MyDataset('/home/zzz/model/Meta-Llama-3-8B-Instruct/original/tokenizer.model', max_seq_len)
    train_dataset, val_dataset = torch.utils.data.random_split(dataset, [0.9, 0.1])
    train_loader = DataLoader(train_dataset, batch_size=max_batch_size, shuffle=True)
    eval_loader = DataLoader(val_dataset, batch_size=max_batch_size, shuffle=True)
